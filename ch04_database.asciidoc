...
We think ChEMBL is preffred for QSAR analysis because ChEMBL provides many dose response data such as IC50 and user can access to original journal for QSAR model interpretation
...
////
There are 259 compounds in the result. All data can see by scrolling the screen. And data can be downloaded as CSV, TSV and SDF format when you click the icon which is located on top right side of the screen. (TSV: tab separated, CSV: camma separated)
